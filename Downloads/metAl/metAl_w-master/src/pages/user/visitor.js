...
import { saveVisitor, getVisitor } from './../../services/visitors';
...
class Visitor extends Form {
	...
	async populateUser() {
		try {
			...
			const { data: visitor } = await getVisitor(userId);
			console.log("visitor", visitor);
			//const visitor = user[0];
			if (visitor.date) visitor.date = new Date(visitor.date);

			visitor.visitorName = visitor.visitorName;
			visitor.category = visitor.category;
			visitor.reasonVisit = visitor.reasonVisit;
			this.setState({ data: this.mapToViewModel(visitor) });
	...
	schema = Joi.object({
		...
		category: Joi.string(),
		reasonVisit: Joi.string(),
		underSupervision: Joi.string(),
		cardNo: Joi.string(),
		// licenseValidTill: Joi.string(),
		// organizationAName: Joi.string(),
		// organizationAMemberNo: Joi.string(),
		// organizationBName: Joi.string(),
		note: Joi.string(),
	...
	doSubmit = async () => {
		...
		try {
			...
			await saveVisitor(this.state.data, this.state.imageSrc);
				this.props.history.push("/user/visitors");
		...
		// 	  await saveVisitor(data,this.state.imageSrc);
			...
			<React.Fragment>
				<div>
					<ol className="breadcrumb float-xl-right">
						<li className="breadcrumb-item"><Link to="/user/visitors">Visitors</Link></li>
						<li className="breadcrumb-item active">Add Visitor</li>
					</ol>
					<h1 className="page-header">
						Add Visitor <small>Visitor-registration-form</small>
					</h1>

					<div className="row">
						<div className="col-xl-10">
							<Panel>
								<PanelHeader>Add Visitor</PanelHeader>
