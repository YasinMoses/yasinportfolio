...
import { saveEROTL, getEROTL } from './../../services/erotls';
...
class ERO extends Form {
	...
	async populateUser() {
		try {
			...
			const { data: ero } = await getEROTL(userId);
			console.log("ero", ero);
			//const ero = user[0];
	...
	doSubmit = async () => {
		...
		try {
			...
			//await saveEROTL(this.state.data, this.state.imageSrc);
			const data = {...this.state.data};
			const user = auth.getProfile();
			  ...
			  await saveEROTL(data,this.state.imageSrc);
