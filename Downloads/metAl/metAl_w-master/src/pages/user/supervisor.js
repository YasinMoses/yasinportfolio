...
import { saveSupervisor, getSupervisor } from './../../services/supervisors';
import { getCompanys } from '../../services/companies.js';
const Handle = Slider.Handle;


class Supervisor extends Form {
	constructor(props) {
		super(props);
	...
	async populateUser() {
		try {
			...
			const { data: supervisor } = await getSupervisor(userId);
			console.log("supervisor", supervisor);
			//const supervisor = user[0];
			if (supervisor.user.dateBirth) supervisor.user.dateBirth = new Date(supervisor.user.dateBirth);

			supervisor.user.firstName = supervisor.user.contactName.first;
			supervisor.user.lastName = supervisor.user.contactName.last;
			supervisor.user.initials = supervisor.user.contactName.initials;
	...
	doSubmit = async () => {
		try {
			...
			await saveSupervisor(this.state.data, this.state.imageSrc)
			this.props.history.push("/user/supervisors");
		...
		// 	  await saveSupervisor(data,this.state.imageSrc);
			...
			<React.Fragment>
				<div>
					<ol className="breadcrumb float-xl-right">
						<li className="breadcrumb-item"><Link to="/user/supervisors">Supervisors</Link></li>
						<li className="breadcrumb-item active">Add Supervisor</li>
					</ol>
					<h1 className="page-header">
						Add Supervisor <small>Supervisor-registration-form</small>
					</h1>

					<div className="row">
						<div className="col-xl-10">
							<Panel>
								<PanelHeader>Add Supervisor</PanelHeader>
