...
import { saveStudent, getStudent } from './../../services/students';
...
class Student extends Form {
	...
	async populateGenders() {
		this.gendstudentptions = this.genderOptions.map(option => (
	...
	async populateUser() {
		try {
			...
			const { data: student } = await getStudent(userId);
			console.log("student", student);
			//const student = user[0];
			if (student.user.dateBirth) student.user.dateBirth = new Date(student.user.dateBirth);

			student.user.firstName = student.user.contactName.first;
			student.user.lastName = student.user.contactName.last;
			student.user.initials = student.user.contactName.initials;
	...
	doSubmit = async () => {
		...
		try {
			console.log(this.state.data);
			await saveStudent(this.state.data, this.state.imageSrc);

			this.props.history.push("/user/students");
		} catch (ex) {
			...
			<React.Fragment>
				<div>
					<ol className="breadcrumb float-xl-right">
						...
						<li className="breadcrumb-item"><Link to="/user/students">Students</Link></li>
						<li className="breadcrumb-item active">Add Student</li>
					</ol>
					<h1 className="page-header">
						Add Ero <small>Student-registration-form</small>
					</h1>
