...
import { saveSchool, getSchool } from "./../../services/schools";
...
class School extends Form {
  ...
  async populateUser() {
    try {
      ...
      const { data: user } = await getSchool(userId);
      //console.log(user[0]);
      //const School = Object.assign({},user[0]);
      const School = user[0];
      console.log("School by id",School);
      School.firstName = School.contactName.first;
      School.lastName = School.contactName.last;
      School.initials = School.contactName.initials;
      School.IBAN = School.bankInfo.IBAN;
      School.bank = School.bankInfo.bank;
      School.branchOfBank = School.bankInfo.branchOfBank;
      School.subscription = School.subscription.subscription;
      School.subscriptionPeriod = School.subscription.subscriptionPeriod||"month";	  
      School.businessName = School.companyInfo.businessName;
      School.chamberCommerceNo = School.companyInfo.chamberCommerceNo;
      School.taxPayerNo = School.companyInfo.taxPayerNo;
      School.website = School.companyInfo.website;
      School.size = School.companyInfo.size;
      // School.industry = School.companyInfo.industry;
      School.licenseNo = School.professionalInfo.licenseNo;
      School.licenseValidTill = School.professionalInfo.licenseValidTill;
      School.organizationAName = School.membership.organizationAName;
      School.organizationAMemberNo = School.membership.organizationAMemberNo;
      School.organizationBName = School.membership.organizationBName;
      School.organizationBMemberNo = School.membership.organizationBMemberNo;
      // School.subscriptionStartDate = (School?.subscription?.subscriptionStartDate)? School.subscription.subscriptionStartDate:new Date();
      if (School.workingHours && School.workingHours.length > 0) {
        this.state.workingHours = School.workingHours;
    }
      console.log(this.state.workingHours);
      
      //  delete School.password;

      this.setState({ data: this.mapToViewModel(School) });
      console.log(School);
  ...
  doSubmit = async (user) => {
    ...
    try {
      ...
      await saveSchool(dataToSubmit, this.state.data.imageSrc);
      //console.log(this.state.data);
      this.props.history.push("/martial/schools");
    } catch (ex) {
      //if(ex.response && ex.response.status === 404){
      if (ex.response) {
        const errors = { ...this.state.errors };
      ...
      <React.Fragment>
        <div>
          ...
          <h1 className="page-header">
            Add School <small>School-registration-form</small>
          </h1>

          <div className="row">
            <div className="col-xl-10">
              <Panel>
                <PanelHeader>Add School</PanelHeader>
                      ...
                      <div className="btn-group col-lg-8">
                        <div className="custom-control custom-radio custom-control-inline">
                          <input
                            ...
                            value="School"
                            checked={data.subscription === "School"}
                          />
                          <label
