...
import {saveAthlete,getAthlete} from './../../services/athletes';
...
class Athlete extends Form {
	...
	async populateUser() { 
		try {
		  ...
		  const { data: user } = await getAthlete(userId);
            const athlete = user[0];
		    console.log(athlete);

			
        //athlete.dateBirth = new Date(athlete.dateBirth);
        if(athlete.dateBirth)athlete.dateBirth = moment(new Date(athlete.dateBirth)).format("YYYY-MM-DD");
      else athlete.dateBirth = new Date();
		     

			athlete.firstName =athlete.contactName.first;
			athlete.lastName =athlete.contactName.last;
			athlete.initials =athlete.contactName.initials;
			// athlete.IBAN = athlete.bankInfo.IBAN;
			// athlete.bank = athlete.bankInfo.bank;
			// athlete.branchOfBank = athlete.bankInfo.branchOfBank;
			// athlete.healthcareProviderIdentifierOrganisation = athlete.professionalInfo.healthcareProviderIdentifierOrganisation;
			// athlete.healthcareProviderIdentifierIndividual = athlete.professionalInfo.healthcareProviderIdentifierIndividual;
			// athlete.treatments = athlete.professionalInfo.treatments;
			// athlete.licenseNo  = athlete.professionalInfo.licenseNo;
			// athlete.licenseValidTill = athlete.professionalInfo.licenseValidTill;
			// athlete.organizationAName = athlete.membership.organizationAName;
			// athlete.organizationAMemberNo = athlete.membership.organizationAMemberNo;
			// athlete.organizationBName =athlete.membership.organizationBName;
			// athlete.organizationBMemberNo =athlete.membership.organizationBMemberNo;
			// athlete.idPaper  =athlete.identification.idPaper;
			// athlete.idPaperValidTill =athlete.identification.idPaperValidTill;

		  this.setState({ data: this.mapToViewModel(athlete) });
...
schema = Joi.object({
	...
	doSubmit = async (user) => {
	    ...
	    try{
			console.log(this.state.data);
			await saveAthlete(this.state.data,this.state.imageSrc);
			//console.log(this.state.data);
			this.props.history.push("/martial/athletes");
		}catch(ex){
			//if(ex.response && ex.response.status === 404){
			if(ex.response){
				const errors = {...this.state.errors};
			...
			<React.Fragment>
				<div>
					<ol className="breadcrumb float-xl-right">
						<li className="breadcrumb-item"><Link to="/martial/athletes">Athletes</Link></li>
						<li className="breadcrumb-item active">Add Athlete</li>
					</ol>
					<h1 className="page-header">
						Add Athlete <small>Athlete-registration-form</small>
					</h1>

					<div className="row">
						<div className="col-xl-10">
							<Panel>
								<PanelHeader>Add Athlete</PanelHeader>
