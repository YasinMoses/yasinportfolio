...
import {saveExaminator,getExaminator} from './../../services/examinators';
...
class Examinator extends Form {
	...
	async populateUser() { 
		try {
		  ...
		  const { data: user } = await getExaminator(userId);
            const examinator = user[0];
		    console.log(examinator);
			//if(examinator.dateBirth)examinator.dateBirth = new Date();  
			if(examinator.dateBirth)examinator.dateBirth = moment(examinator.dateBirth).format("YYYY-MM-DD");
			else examinator.dateBirth = new Date(); 
			examinator.firstName =examinator.contactName.first;
			examinator.lastName =examinator.contactName.last;
			examinator.initials =examinator.contactName.initials;
			// examinator.address1 = examinator.Address.address1;
			// examinator.address2 = examinator.Address.address2;
			// examinator.address3 = examinator.Address.address3;
			// examinator.zip = examinator.Address.zip;
			// examinator.city = examinator.Address.city;
			// examinator.state = examinator.Address.state;				
			// examinator.country = examinator.Address.country;
			examinator.IBAN = examinator.bankInfo.IBAN;
			examinator.bank = examinator.bankInfo.bank;
			examinator.branchOfBank = examinator.bankInfo.branchOfBank;
			examinator.treatments = examinator.professionalInfo.treatments;
			examinator.licenseNo  = examinator.professionalInfo.licenseNo;
			if(examinator.licenseValidTill)examinator.licenseValidTill = moment(examinator.licenseValidTill).format("YYYY-MM-DD");
			else examinator.licenseValidTill = new Date();  
			examinator.organizationAName = examinator.membership.organizationAName;
			examinator.organizationAMemberNo = examinator.membership.organizationAMemberNo;
			examinator.organizationBName =examinator.membership.organizationBName;
			examinator.organizationBMemberNo =examinator.membership.organizationBMemberNo;
			examinator.idPaper  =examinator.identification.idPaper;
			if(examinator.idPaperValidTill)examinator.idPaperValidTill = moment(examinator.idPaperValidTill).format("YYYY-MM-DD");
			else examinator.idPaperValidTill = new Date();  
		  this.setState({ data: this.mapToViewModel(examinator) });
...
schema = Joi.object({
	...
	doSubmit = async (user) => {
	    ...
	    try{
			console.log(this.state.data);
			await saveExaminator(this.state.data,this.state.imageSrc);
			//console.log(this.state.data);
			this.props.history.push("/martial/examinators");
		}catch(ex){
			//if(ex.response && ex.response.status === 404){
			if(ex.response){
				const errors = {...this.state.errors};
			...
			<React.Fragment>
				<div>
					<ol className="breadcrumb float-xl-right">
						<li className="breadcrumb-item"><Link to="/martial/examinators">Examinators</Link></li>
						<li className="breadcrumb-item active">Add Examinator</li>
					</ol>
					<h1 className="page-header">
						Add Examinator <small>Examinator-registration-form</small>
					</h1>

					<div className="row">
						<div className="col-xl-10">
							<Panel>
								<PanelHeader>Add Examinator</PanelHeader>
