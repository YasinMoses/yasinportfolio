...
import { saveEvent, getEvent } from "./../../services/events";
...
//import Actions from "./../../../src/components/event/Action";
//import Card from "./../../../src/components/event/Card";
//import Filter from "./../../../src/components/event/Filters";
//import Category from "./../../../src/components/event/Category";
...
class Eventprofile extends React.Component {
  ...
  async populateEvent() {
    try {
      ...
      let sharedusers = [];
      event.share.sharedTo.map((shareduser, index) => {
        let sharedobject = {
          userid: shareduser._id,
          email: shareduser.email,
          username: shareduser.username,
          sharedtilldate: event.share.sharedTill[index],
          view: event.share.permissions[index].view,
          comment: event.share.permissions[index].comment,
          edit: event.share.permissions[index].edit,
        };
        sharedusers.push(sharedobject);
      });
      this.setState({ data: this.mapToViewModel(event, sharedusers) });
  ...
  handleSubmit = async () => {
    ...
    try {
      console.log(this.state.data);
      await saveEvent(this.state.data, this.state.imageSrc);
      this.props.history.push("/event/events");
    } catch (ex) {
      if (ex.response) {
  ...
  mapToViewModel(event, sharedusers) {
    return {
      _id: event._id,
  ...
  onClickHandler = async (e) => {
    ...
    try {
      ...
      //this.props.history.push("/clinic/events");
    } catch (ex) {
      if (ex.response) {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <h1 className="page-header">Event-profile</h1>
        <div className="row">
          <div className="col-12">
            <Panel>
              <PanelHeader noButton>Events</PanelHeader>
              <PanelBody>
                <h1>Event name</h1>
                ...
                <TabContent activeTab={this.state.activeTab}>
                  ...
                  <ReusableTab id={6}>
                    <>
                      ...
                      <p>
                        ...
                        to use and work on are located in src/page/event When
                  ...
                  <ReusableTab id={8}>
                    <>
                      <h4>Pie-chart of events</h4>
                      <p>
                        pie-chart with events on it
                        ...
                        is a category of events. Each sections has its own
                        color Display a event on the pie-chart. Place it in the
                        right section. Create a pie-chart for events. Retrieve
                        data from table events and display them on the
                        pie-chart. The events will be divided into 8
                        ...
                        event-nr, username of the creator and username of the
